...
#define QUANTUM_IMPLEMENTATION(namespace) \
    ...
    controlled_z = namespace::sigma_z,    \
...
namespace quantum {
    ...
    void (*controlled_z) (const size_type, const size_type, quregiser& quregister&);
    void (*kronecker) (quregister&, quregister&);
    void (*measure) (const size_type, const real, quregister&, quregister&)
    
    implementation (std::string imp) {
        if (imp == "openmp")
            QUANTUM_IMPLEMENTATION (openmp);
        if (imp == "sequential")
            QUANTUM_IMPLEMENTATION (sequential);
        if (imp == "tbb")
            QUANTUM_IMPLEMENTATION (tbb);
    }
